...
var cached = global.payload; // Se crea una variable cached que almacena el cliente y la promesa de inicialización. 
            ...
            switch (_d.label) {
                case 0:
                    ...
                    if (cached.client) { // Si el cliente ya está en caché, 
                        return [2 /*return*/, cached.client]; // se devuelve directamente sin realizar una nueva inicialización.
                    }
                    if (!cached.promise) { // Si no hay una promesa existente, 
                        cached.promise = payload_1.default.init(__assign(__assign({ secret: process.env.PAYLOAD_SECRET, local: (initOptions === null || initOptions === void 0 ? void 0 : initOptions.express) ? false : true }, (initOptions || {})), { email: {
                ...
                case 1:
                    _d.trys.push([1, 3, , 4]);
                    _c = cached;
                    return [4 /*yield*/, cached.promise]; // Intenta obtener el cliente desde la promesa
                case 2:
                    _c.client = _d.sent(); // Intenta obtener el cliente desde la promesa
                    return [3 /*break*/, 4];
                case 3:
                    e_1 = _d.sent();
                    cached.promise = null; // Si hay un error, se limpia la promesa para que se intente nuevamente en el próximo llamado.
                    return [3 /*break*/, 4];
                case 4: return [2 /*return*/, cached.client]; // Finalmente, devuelve el cliente de Payload que ha sido almacenado en caché.
            }
        });
    });
};
